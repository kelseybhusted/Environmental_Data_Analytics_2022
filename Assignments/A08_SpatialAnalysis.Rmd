...
NWIS.SiteFlow.join <- NWIS.GageLocation.sf %>%
  left_join(NWIS.SiteFlow, by = c("site_no" = "site_no"))
...
ggplot() +
  ...
  geom_sf(data = NWIS.SiteFlow.join, aes(size='gage_ht'))

#https://stackoverflow.com/questions/63351058/geom-sf-and-geom-point-how-to-assign-colors-based-on-the-points-inserted-on-the
```
...
#21 Plot the data
ggplot() +
  geom_sf(data = NE_counties_sf.utm, color = 'blue') +
  geom_sf(data = NWIS.GageHeightLocation.utm, zcol = 'gage_ht', shape = 'O')
...
ggplot() +
  ...
  geom_sf(data = LancasterCounty, fill = 'green') +
