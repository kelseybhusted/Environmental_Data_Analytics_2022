...
```
> Answer:Factors are generally used to represent categorical data. Although factors look like character vectors, they are actually integers that are represented by characters. Looking at the Conc.1..Author column, the columnn contains charcters in addition to numeric variable. Therefore, Conc.1..Author cannot be classified as numerical or categorical.
...
<<<<<<< HEAD:A03_DataExploration_Revised.Rmd
```{r}
ggplot(Neonics) +
...
> Answer: According to the graph displayed above, the most common test locations include the lab and natural field. While tests conducted within the natural field seem to initially increase and then noticeably decrease with time, tests performed in the lab display more of a general increase over time. 
