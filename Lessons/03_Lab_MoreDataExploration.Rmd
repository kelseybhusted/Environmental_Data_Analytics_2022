...
matrix1[4, ] #presents 4th row in matrix1
matrix1[ , 3] #presents the 3rd column in matrix1
matrix1[c(12, 14)] #presents 12 and 14
matrix1[c(12:14)] #presents a range 12 to 14 in matrix1
matrix1[2:4, 1:3] #
...
dataframe1[1:2,] # shows only rows 1 and 2
dataframe1[c("ID","Passed")] #shows ID and Passed columns only
dataframe1$ID
...
high_tempF <- c(53, 45, 66, 36, 57, 42, 39, 55, 58, 60)

low_tempF <- c(30, 27, 25, 26, 35, 23, 27, 31, 20, 22)
```
...
conversion_F_to_C <- function(x) {
  (x-32)/1.8
}

high_tempC <- conversion_F_to_C(high_tempF)
high_tempC

low_tempC <- conversion_F_to_C(low_tempF)
low_tempC
...
```{r}
dataframe_temps <- data.frame(high_tempF, high_tempC, low_tempF, low_tempC)
dataframe_temps
names(dataframe_temps) <- c("High Temps. F", "High Temps. C", "Low Temps. F", "Low Temps. C"); View(dataframe_temps)
...
```{r}
summary(dataframe_temps) #summary statistics for entire data frame
summary(dataframe_temps$`High Temps. F`) #summary statistics for a single column


sd(dataframe_temps$`High Temps. F`)
