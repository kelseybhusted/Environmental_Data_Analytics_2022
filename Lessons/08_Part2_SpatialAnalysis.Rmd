...
huc8.UpperNeuse_sf <- st_read('./Data/Spatial/NCHUC8.shp') %>%
  filter(SUBBASIN == 'Upper Neuse')

#Create a map
mapview(huc8.UpperNeuse_sf, col.regions = 'blue') +
...
view(nc_powerplants_sf)
nc_powerplants_sf %>% 
  st_drop_geometry() %>% 
  count(TYPE, sort=TRUE)

#Most are solar farms; let's remove those
#filtering out solar photovoltaic 
...
counties_sf_join <-  merge(x = counties_sf,
                           y = svi2018_nc_raw, 
                           #GEOID and FIPS columns are the same in each data set so must join with them
                           by.x = "GEOID", 
                           by.y = "FIPS" )
...
wind_energy_huc8 <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',colClasses = c('HUC_12' = 'factor')) %>%
  group_by(HUC_8 = substr(HUC_12,1,8)) %>%
  summarize(kWhkm2day = sum(kWhkm2day))
view(wind_energy_huc8)
...
#View the outputs
mapview(huc8_sf_wind, zcol = 'kWhkm2day')

ggplot() +
  geom_sf(data = huc8_sf_wind, aes(col = 'kWhkm2day'))
...
#selecting Neuse basin only inthe following code
neuse_sf <- huc2_utm %>% 
...
#select counties that intersect the Neuse Basin
#taking all 100 NC counties and finding the ones that intersect with the Neuse River
neuse_intersect_1 <- counties_sf_utm[neuse_sf,]

#Another way of filtering for intersecting counties
...
mapview(the_triangle)
  
#Grab the intersecting HUC_8s
the_hucs <- huc8_sf_utm[the_triangle,]

mapview(the_hucs, alpha.regions = 0) + mapview(the_triangle)
view(the_hucs)

#Intersect the HUC_8s
the_hucs_clipped <- the_hucs %>%
  st_intersection(the_triangle)
mapview(the_hucs_clipped, zcol = 'HUC_8')
