...
#cycle is like a seasonal component but will repeat over longer time-periods like climate change trends
...
```{r}
#in a rime series, it's important to define a starting point; in this particular case, first observation point is in January 1985
#frequency of observation is 365
view(EnoDischarge.late)
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365)
#in code below, looking at the first 10 elements of the discharge mean
...
#we're creating a new data frame
EnoDischarge.late_Components <- as.data.frame(EnoDischarge.late_Decomposed$time.series[,1:3])

#adding two more columns to my components (i.e, Date and Observed columns)
...
view(EnoDischarge.late_Components)
...
wind_data_clean <- 
  wind_data %>% 
  #adding a new column called 'AWND.clean' with the zoo package
  #na.approx interpolates missing data using either the linear interpolation or the sp line interpolation and applying it to column 'AWND'
  mutate( AWND.clean = zoo::na.approx(AWND) )

summary(wind_data_clean$AWND.clean)
#Note the NA is gone but the the mean, meadian, ect. remians the same
...
#from the decompose (i.e., stl) test, we know that we're dealing with seasonality so we use the Seasonal Mann Kendall test to look at the monotonic trend
...
#Another way to run the Seasonal Mann Kendall Test
#shows the results for each individual season of the year
