...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #The dataset needs to be stated first
  mutate(month = month(sampledate)) %>% #adding a month column 
  select(lakeid:daynum, month, sampledate:comments) %>% # rearranging the column order
  drop_na(depth)
...
#subset does something similar to filtering; !is.na removes rows with NA in the concentration column
...
view(NTL.nutrient.data.PeterPaul.spread2)
...
#reading each element in the file
Litter <- LitterFiles %>%
  ldply(read.csv)
```
