...
#same plot as above but in a different way
#useful when wanting to add two plots in the same window or page; you should get used to practicing storing plots into a name
...
#for shape = as.factor(), automatically asking r to change the shape of the dots based on the year!
PMplot <- 
...
#facet_wrap makes different graphs for each site with the dataset
PMplot.faceted <-
...
#subset data for on specified sites
#facet_wrap Site.Name are the rows, and the Years are the columns
PMplot.faceted2 <- 
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# Scatterplot
#don't have dates in the axis anymore so looking at two variables in relation to each other
lightvsDO <- 
...
#want the deepest observations at the lowest point on graph
tempvsdepth <- 
...
#geom_abline used when you know the actual slope of the line and can input that data
NvsP <-
...
# same as above but slightly different way to code it
Nutrientplot3V2 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration, color = nutrient)) +
  geom_boxplot() # Why didn't we use "fill"?
print(Nutrientplot3V2)
