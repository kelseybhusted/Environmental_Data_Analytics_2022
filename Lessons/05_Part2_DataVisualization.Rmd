...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #lty specifies the  types of line; 2 is a dashed line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + #alpha fixes the transparency of the points
  ...
  ylab(expression("O"[3]* " AQI Value")) + #changing y axis name
...
NvsP2 <-
  ...
  #changing shape of points;since you have two variables need to pick two shapes (i.e., 15 and 17)
  xlab("TP ug/L") + ylab("TN ug/L") +
...
Nutrientplot <-
  ...
  geom_boxplot() +
  scale_fill_discrete(breaks = c("NH34", "NO23", "PO4", "TN (ug)", "TP (ug)"))+
...
Nutrientplot2 <-
  ...
  ylab("Concentration (ug/L)") +
  scale_y_continuous(expand = c(0, 0)) +
  scale_color_brewer(palette = "YlGnBu", labels = c("NH34", "NO23", "PO4", "TN (ug)", "TP (ug)")) +
...
library(cowplot)
#rel_heights() function means adjusting height of plots if on needs more room than the other
