...
#or
NTL.phys.data.1999_data <- filter(NTL.phys.data, year4 == 1999)
...
#does the same thing as the code above just another way to do it :)
NTL.phys.data.TuesdayLake_1990_to_1999V2 <- filter(NTL.phys.data, year4 %in% c(1990:1999))
...
NTL.phys.data.irradience.gather <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Irradiance Types", values_to = "Irradiance Values")


#Exercise 6: spread temperatureC into more than one column based on the depth

NTL.phys.data.temp.spread <- pivot_wider(NTL.phys.data, names_from = "depth", values_from = "temperature_C")
