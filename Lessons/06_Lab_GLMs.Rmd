...
> Answer: The mean is greater than 50 (the null hypothesis)
...
#result: reject the null hypothesis the daya is normally distibuted
...
O3.onesample
#result: reject the null hypothesis (i.e., true mean is greater than 50)
...
> A significant portion of the ozone layer levels are located between the AQI index 0-50 thus the null hypothesis is rejected.
...
#result: reject both null hypothesis (i.e., both years are not normally distributed)
...
O3.twosample
#result: the means are not the same (we reject the null hypothesis)
...
#2
Plot2 <- 
  ggplot(EPAair, aes(x = Year, y = Ozone, fill = as.factor(Year))) +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5,
               label = c("2018", "2019")) +
  scale_fill_manual(values = c("yellow", "orange")) +
  labs(x = "Year", y = "Ozone AQI Value", fill = "") +
  geom_boxplot()
print(Plot2)

#3
#geom_violin of ozone divided by year, with the 0.5 quantile marked as a horizontal line. Add letters representing a significant difference between 2018 and 2019. 
Plot3 <- 
  ggplot(EPAair, aes(x = as.factor(Year), y = Ozone, fill = as.factor(Year))) +
  labs(x = "Year", y = "Ozone AQI Value", fill = "") +
  scale_fill_manual(values = c("lightcyan", "chocolate")) +
...
PM2.5_Ozone.Regressions <- lm (data = EPAair, PM2.5 ~ Ozone )
summary(PM2.5_Ozone.Regressions)
...
PM2.5byOzone.Info <- 
  EPAair %>%
  select(Ozone, PM2.5, Month, Year, meanLat, meanLng) %>%
  na.omit()
print(PM2.5byOzone.Info)

view(EPAair)

PM2.5byOzoneCorr <- cor(PM2.5byOzone.Info)
corrplot(PM2.5byOzoneCorr, method = "ellipse")
corrplot.mixed(PM2.5byOzoneCorr, upper = "ellipse")
...
PM2.5byOzone.AIC <- lm(data = PM2.5byOzone.Info, Ozone ~ PM2.5 + Month + Year + meanLat + meanLng)
summary(PM2.5byOzone.AIC)
step(PM2.5byOzone.AIC)
...
PM2.5byOzone.AIC.filtered <- lm(data = PM2.5byOzone.Info, Ozone ~ PM2.5 + Month + Year + meanLng)
summary(PM2.5byOzone.AIC.filtered)
```
